...
* 零拷贝：会让我们节省一步copy过程，正常情况我们要复制一份文件需要1.从磁盘把数据加载到系统内存2.从系统内存复制一份到应用内存，3.从应该用内存复制到socket缓冲区,然后才能把这个文件再我们的应用里从网络上发送出去，如果用了零拷贝我们就可以省略掉第2步直接把数据从内存copy到socke缓冲区，由于省略了第二步我们就不许要cpu再参与运算了直接用DMA处理器就可以完成操作，使效率更高

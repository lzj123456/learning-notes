...
加载：虚拟机规范里规定根据“通过一个类描述的全限定名找到指定文件以二进制流的方式加载进来”，因此加载一个类我们可以通过类加载器从任何地方加载。加JVM参数-verbose:class打印类加载卸载日志

验证：验证二进制流文件的格式，比如魔术是否是CAFEBABE，版本号是否在JVM能处理的范围等。

准备：为类变量(static修饰的变量)赋初始值为0的操作并分配内存

解析：把常量池中的符号引用转换为直接引用(相当于把符号引用换成指向具体内存的指针)

初始化：执行<clinit>()，这个方法是由静态块与静态变量组成的，执行了静态块中的代码并未执行了静态变量赋值的操作。在此期间<clinit>()方法的执行时加锁同步的，所以才有了用类加载实现线程安全的单例模式。类再第一次使用时才会触发初始化阶段，执行静态代码块。
